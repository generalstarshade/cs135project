...
therows AS (SELECT per.id AS cid, per.person_name AS name, pp.id as pid, pp.product_name, coalesce(proSales.sales, 0) AS total
...
almostfinalstuff AS(
SELECT DISTINCT cid FROM therows 
),

finalstuff AS(
SELECT ROW_NUMBER() OVER() AS theid FROM almostfinalstuff
)

SELECT COUNT(*) FROM finalstuff 
WHERE theid > 20 * ?
...
allSalesNoLimits AS (SELECT per.id AS cid, per.person_name, pp.id as pid, pp.product_name, coalesce(proSales.sales, 0) AS total,
pp.ptotal
FROM proNoLimits pp CROSS JOIN person per
LEFT OUTER JOIN proSalesNoLimits proSales
ON (pp.id = proSales.proid AND per.id = proSales.pid) 
WHERE per.role_id = 2), -- select * from allSalesnolimits

-- Table: ordered
ordered AS (SELECT cid, SUM(total) AS sumTotal
FROM allSalesNoLimits
GROUP BY cid order by sumtotal desc)
